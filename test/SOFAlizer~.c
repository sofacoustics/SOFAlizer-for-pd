...
/********************************************************************************************************************/
	...
	mysofa_getfilter_float(hrtf, values[0], values[1], values[2], leftIR, rightIR, &leftDelay, &rightDelay);
    ...
    while (blocksize--)
    {
		...
		{ 
			convSum[0] += (x->previousImpulse[0][i] * x->crossCoef[blocksizeDelta] + 
							leftIR[i] * x->crossCoef[scaledBlocksize]) * 
							x->convBuffer[(x->bufferPin - i) &127];
			convSum[1] += (x->previousImpulse[1][i] * x->crossCoef[blocksizeDelta] + 
							rightIR[i] * x->crossCoef[scaledBlocksize]) * 
							x->convBuffer[(x->bufferPin - i) &127];

			x->previousImpulse[0][i] = leftIR[i];
			x->previousImpulse[1][i] = rightIR[i];
		}	
